...
//	       dimensions_of_signal = MyInput.readInt(); 
	     ...
	     System.out.println("输入拓扑结构，如10-10");
//	     String[] split = MyInput.readString().split("-");
...
//	     int y=Integer.parseInt(split[1]);
	     ...
	     maximum_number_of_clusters = x*y;//簇个数 
	     System.out.println();

	     
	     
	     // 建立SOM竞争层，
	     //将这个竞争层改为两维的，使得最后结果分布在二维平面上
	    int row=0;//行
		int col=0;//列
	...
	void kluster_nodes_compete_for_activation(){
	    ...
	    minDistances.add(minimum_distance);//将最大相似度的最近距离保存起来
	  } 
